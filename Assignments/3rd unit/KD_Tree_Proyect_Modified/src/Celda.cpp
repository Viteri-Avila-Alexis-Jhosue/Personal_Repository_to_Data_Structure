...
float Celda::ingresar_coordenada(int size, int coord, int contador){
    string a,coordenada;
    if(coord==0){
        coordenada="x";
    }else{
    ...
    do {
        srand(time(0)+ contador); // Inicializa la semilla para el generador de números aleatorios
        a = to_string(rand() % size); // Asigna un número aleatorio dentro del rango
        if (stof(a) >= size || stof(a) < 0) {
            cout << "Coordenada fuera del rango " << endl;
        }
    } while (stof(a) >= size || stof(a) < 0);
    return stof(a);
}
float Celda::ingresar_coordenada_no_randomica(int size, int coord){
    string a,coordenada;
