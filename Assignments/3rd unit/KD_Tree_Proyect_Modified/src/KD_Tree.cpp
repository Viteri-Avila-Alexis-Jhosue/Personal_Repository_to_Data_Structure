...
bool KD_Tree<T>::verificar_espacio_en_parqueadero_rec(const std::shared_ptr<Nodo<T>>& node) const {
    ...
    if (node->data.getHoraSalida() != fecha_improbable) {
        return true; // Se encontró un nodo con espacio disponible
    }
...
template <typename T>
int KD_Tree<T>::getDepth() const {
    return getDepthRec(root, 0); // La raíz comienza con profundidad 0
}

template <typename T>
int KD_Tree<T>::getDepthRec(const std::shared_ptr<Nodo<T>>& node, int currentDepth) const {
    if (node == nullptr) return currentDepth - 1; // Restamos 1 para no contar el nodo nulo

    int leftDepth = getDepthRec(node->left, currentDepth + 1);
    int rightDepth = getDepthRec(node->right, currentDepth + 1);

    return std::max(leftDepth, rightDepth);
}

template <typename T>
int KD_Tree<T>::getHeight() const {
    return getHeightRec(root);
}

template <typename T>
int KD_Tree<T>::getHeightRec(const std::shared_ptr<Nodo<T>>& node) const {
    if (node == nullptr) return -1; // Un árbol vacío tiene altura -1
...
int KD_Tree<T>::getNodeLevel(const std::shared_ptr<Nodo<T>>& target) const {
    return getNodeLevelRec(root, target, 0);
}

template <typename T>
int KD_Tree<T>::getNodeLevelRec(const std::shared_ptr<Nodo<T>>& current, const std::shared_ptr<Nodo<T>>& target, int level) const {
    if (current == nullptr) return -1; // Nodo no encontrado

    if (current == target) return level;

    int leftLevel = getNodeLevelRec(current->left, target, level + 1);
    if (leftLevel != -1) return leftLevel;

    return getNodeLevelRec(current->right, target, level + 1);
}
template <typename T>
std::shared_ptr<Nodo<T>> KD_Tree<T>::buscarPorCoordenadasEnCeldaRec(std::shared_ptr<Nodo<T>> node, int x, int y) const {
    if (node == nullptr) {
        return nullptr; 
    }
    ...
    auto encontradoIzquierda = buscarPorCoordenadasEnCeldaRec(node->left, x, y);
    if (encontradoIzquierda != nullptr) {
        return encontradoIzquierda;
    }
    return buscarPorCoordenadasEnCeldaRec(node->right, x, y);
}

template <typename T>
std::shared_ptr<Nodo<T>> KD_Tree<T>::buscarPorCoordenadasEnCelda(int x, int y) const {
    return buscarPorCoordenadasEnCeldaRec(root, x, y);
}
//--------------------------------------------------------------------------------------------------------
template <typename T>
void KD_Tree<T>::inOrderTraversal() const {
    inOrderTraversalRec(root);
}

template <typename T>
void KD_Tree<T>::inOrderTraversalRec(const std::shared_ptr<Nodo<T>>& node) const {
    if (node == nullptr) return;

    inOrderTraversalRec(node->left);
    std::cout << "Coordenadas: (" << node->data.getX() << ", " << node->data.getY() << "), Placa: " << node->data.getCoche().getPlaca() << std::endl;
    inOrderTraversalRec(node->right);
}

template <typename T>
void KD_Tree<T>::preOrderTraversal() const {
    preOrderTraversalRec(root);
}

template <typename T>
void KD_Tree<T>::preOrderTraversalRec(const std::shared_ptr<Nodo<T>>& node) const {
    if (node == nullptr) return;
...
template <typename T>
void KD_Tree<T>::postOrderTraversal() const {
    postOrderTraversalRec(root);
}

template <typename T>
void KD_Tree<T>::postOrderTraversalRec(const std::shared_ptr<Nodo<T>>& node) const {
    if (node == nullptr) return;
...
template <typename T>
std::array<float, 2> KD_Tree<T>::buscarCeldaVacia(int size) const {
    ...
    do {
        // Generar coordenadas aleatorias dentro del rango permitido
        coordenadas[0] = dist(gen);
        coordenadas[1] = dist(gen);
    } while (existeCeldaOcupada(root, coordenadas[0], coordenadas[1]));

    return coordenadas;
}

template <typename T>
bool KD_Tree<T>::existeCeldaOcupada(const std::shared_ptr<Nodo<T>>& node, float x, float y) const {
    if (node == nullptr) return false;

    // Obtener la fecha improbable como referencia
    std::chrono::system_clock::time_point fecha_improbable = definirFechaImprobable();

    // Verificar si el nodo actual tiene las mismas coordenadas y está ocupado
    if (node->data.getX() == x && node->data.getY() == y && node->data.getHoraSalida() == fecha_improbable) {
        return true;
    }
...
template <typename T>
void KD_Tree<T>::obtener_niveles(const std::shared_ptr<Nodo<T>>& node, std::vector<std::vector<std::shared_ptr<Nodo<T>>>>& niveles) const {
    if (!node) return;

    std::queue<std::shared_ptr<Nodo<T>>> cola;
    cola.push(node);

    while (!cola.empty()) {
        int nivel_size = cola.size();
        std::vector<std::shared_ptr<Nodo<T>>> nivel_actual;
