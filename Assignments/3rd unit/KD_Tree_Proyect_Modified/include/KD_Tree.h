...
private:
    ...
    bool verificar_espacio_en_parqueadero_rec(const std::shared_ptr<Nodo<T>>& node) const;
    int getDepthRec(const std::shared_ptr<Nodo<T>>& node,int currentDepth) const;
    int getHeightRec(const std::shared_ptr<Nodo<T>>& node) const;
    int getNodeLevelRec(const std::shared_ptr<Nodo<T>>& current, const std::shared_ptr<Nodo<T>>& target, int level) const;
    std::shared_ptr<Nodo<T>> buscarPorCoordenadasEnCeldaRec(std::shared_ptr<Nodo<T>> node, int x, int y) const;
    void inOrderTraversalRec(const std::shared_ptr<Nodo<T>>& node) const;
    void preOrderTraversalRec(const std::shared_ptr<Nodo<T>>& node) const;
    void postOrderTraversalRec(const std::shared_ptr<Nodo<T>>& node) const;
    bool existeCeldaOcupada(const std::shared_ptr<Nodo<T>>& node, float x, float y) const;
    void graficar_arbol_rec(const std::shared_ptr<Nodo<T>>& node, int depth) const;
    void obtener_niveles(const std::shared_ptr<Nodo<T>>& node, std::vector<std::vector<std::shared_ptr<Nodo<T>>>>& niveles) const;
    void imprimir_nivel(const std::vector<std::shared_ptr<Nodo<T>>>& nivel, int depth, int nivel_actual) const;
...
public:
    ...
    bool verificar_espacio_en_parqueadero() const;
    int getDepth()const;
    int getHeight()const;
    int getNodeLevel(const std::shared_ptr<Nodo<T>>& target) const;
    std::shared_ptr<Nodo<T>> buscarPorCoordenadasEnCelda(int x, int y) const;
