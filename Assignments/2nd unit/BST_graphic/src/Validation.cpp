...
int Validation::ingresarInt(const char* msj) {
    ...
    int i = 0;
    int valor = 0;
    bool inputValid = false; // Para verificar si se ingresó algo válido
    ...
    while (!inputValid) {
        while((c = getch()) != 13) {  // Mientras no se presione Enter
            ...
            } else if (c == 8) {  // Si se presiona retroceso
                if (i > 0) {
                    printf("\b \b");
                    i--;
                }
        ...
        if (i > 0) {  // Verificar si se ha ingresado algún número
            valor = atoi(cad);  // Convertir a entero
            inputValid = true;   // Aceptar la entrada
        } else {
            printf("\nPor favor, ingrese un numero :");
            i = 0;  // Reiniciar el contador para que se ingrese nuevamente
        }
    }
